...
from .networks_3D import InpaintGenerator3D, Discriminator3D
from .loss import AdversarialLoss, PerceptualLoss, StyleLoss
from .models import BaseModel


class InpaintingModel3D(BaseModel):
    def __init__(self, config, run_path, name="InpaintingModel"):
        super(InpaintingModel3D, self).__init__(name, run_path, config)

        self.z_size = config.z_size
        generator = InpaintGenerator3D(use_spectral_norm=True)
        discriminator = Discriminator3D(in_channels=1, use_sigmoid=config.GAN_LOSS != 'hinge')
        if len(config.GPU) > 1:
    ...
    def process(self, images, edges, masks):
        ...
        for z in range(self.z_size):
            gen_content_loss = self.perceptual_loss(outputs[:, :, z, :, :], images[:, :, z, :, :])
        ...
        for z in range(self.z_size):
            gen_style_loss = self.style_loss(
                outputs[:, :, z, :, :] * masks[:, :, z, :, :], images[:, :, z, :, :] * masks[:, :, z, :, :])
